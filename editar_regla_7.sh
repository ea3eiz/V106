...
MARRON="\33[38;5;138m"
						...
						read password                         
                        echo "${AMARILLO}Introduce TGID ej: 4377 "
						read tgid                        
                        echo "${AMARILLO}Introduce TGID por el que quieras salir ej: 214"
						read tgidsalir
                        echo "${BLANCO}Introduce tg conexión ej: 4377 "
						read tgc
                        echo "${CIAN}Introduce tg de desconexión ej: 84377"
                        read tgd
                        echo "${MARRON}Introduce reflector ej: DMR+4377"
                        read ref

sudo sed -i "90c ]," /opt/HBlink3/rules.py
sudo sed -i "91c '$ref': [ " /opt/HBlink3/rules.py                        
sudo sed -i "92c {'SYSTEM': '$ind', 'TS': 2, 'TGID': $tgid, 'ACTIVE': False, 'TIMEOUT': 10, 'TO_TYPE': 'ON',  'ON': [$tgc], 'OFF': [$tgd], 'RESET': []}," /opt/HBlink3/rules.py
sudo sed -i "93c {'SYSTEM': '$ref', 'TS': 2, 'TGID': $tgidsalir, 'ACTIVE': True, 'TIMEOUT': 2, 'TO_TYPE': 'NONE',  'ON': [], 'OFF': [], 'RESET': []}," /opt/HBlink3/rules.py

                        sudo sed -i "720c [$ref] " /opt/HBlink3/hblink.cfg # no tocar 
                        ...
                        sudo sed -i "726c PORT: 54007" /opt/HBlink3/hblink.cfg # #OJO!! CAMBIAR AL QUE CORRESPONDA EN ESTE CASO DIGITOS 54002
						...
						echo "${CIAN}Introduce Id ej: 214317507 9 dígitos " #OJO!! CAMBIAR AL QUE CORRESPONDA EN ESTE CASO DIGITOS 89 = 02
						read id
                        ...
                        sudo sed -i "744c PACKAGE_ID: HBLLINK RASPI" /opt/HBlink3/hblink.cfg #no tocar
